...
class TestFindLightBeginnig(unittest.TestCase):
    def test_1(self):
        dark_beg = "12:00"
        dark_len = 12
        self.assertEqual(ut.find_light_beginning(dark_beg, dark_len), "00:00")

    def test_2(self):
        dark_beg = "12:40"
        dark_len = 12.5
        self.assertEqual(ut.find_light_beginning(dark_beg, dark_len), "01:10")

    def test_3(self):
        dark_beg = "00:00"
        dark_len = 12.5
        self.assertEqual(ut.find_light_beginning(dark_beg, dark_len), "12:30")
...
class DealWithDates(unittest.TestCase):
    ...
    def test_last_day_to_datetime(self):
        ...
        expected = datetime(year=2014, month=6, day=19, hour=11, tzinfo=tzinfo("UTF"))

    def test_strtime_to_datetime(self):
        measured = ut.strtime_to_datetime("%s12:00 UTC" % self.first)
        expected = datetime(year=2014, month=6, day=16, hour=12, tzinfo=tzinfo("UTF"))

    def test_get_date(self):
        self.assertEqual(ut.get_date(date(day=16, month=6, year=2014)), "16.06.2014")

    def test_get_time(self):
        self.assertEqual(ut.get_time(time(hour=12)), "12:00")

    def generate_entry(self):
        first = datetime(year=2014, month=6, day=19, hour=12, tzinfo=tzinfo("UTF"))
        last = datetime(year=2014, month=6, day=20, hour=00, tzinfo=tzinfo("UTF"))
