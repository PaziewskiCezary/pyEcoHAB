...
class TestAntennaTransitions(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.expected_dark = {"1 dark": {0.0: {}}, "1 light": {0.0: {}}}
        ...
        cls.expected["ALL"][0.0]["4 5"] = [
            4.38,
            3.923,
        ]
        cls.expected["ALL"][0.0]["5 6"] = [1.963, 19.035, 1.136, 12.458]
        cls.expected["ALL"][0.0]["6 6"] = [3.882, 0.906, 101.777, 17.995, 11.994]
        cls.expected["ALL"][0.0]["6 5"] = [
            88.224,
            0.734,
            0.547,
        ]
        cls.expected["ALL"][0.0]["5 5"] = [34.989, 67.763, 7.751, 0.752]
        ...
        cls.expected["ALL"][0.0]["2 1"] = [0.891]
        cls.expected["ALL"][0.0]["1 1"] = [8.874]
        cls.expected["ALL"][0.0]["1 2"] = [0.651]
        cls.expected_dark["1 dark"][0.0]["4 4"] = [2.348, 0.25, 18.326]
        cls.expected_dark["1 dark"][0.0]["4 5"] = [
            4.38,
            3.923,
        ]
        cls.expected_dark["1 dark"][0.0]["5 6"] = [1.963, 19.035, 1.136, 12.458]
        cls.expected_dark["1 dark"][0.0]["6 6"] = [
            3.882,
            0.906,
            101.777,
            17.995,
        ...
        cls.expected_dark["1 dark"][0.0]["5 5"] = [34.989, 67.763, 7.751, 0.752]
        ...
        cls.expected_dark["1 dark"][0.0]["2 1"] = [0.891]
        cls.expected_dark["1 dark"][0.0]["1 1"] = [8.874]
        cls.expected_dark["1 dark"][0.0]["1 2"] = [0.651]

        cls.calc = tr.get_antenna_transition_durations(
            cls.data, cls.timeline, binsize="ALL"
        )
        cls.calc_dark = tr.get_antenna_transition_durations(
            cls.data, cls.timeline, binsize=1800
        )
        for key in cls.expected["ALL"][0.0]:
            line = cls.expected["ALL"][0.0][key]
            cls.expected["ALL"][0.0][key] = [np.round(a, decimals=3) for a in line]
            cls.expected_dark["1 dark"][0.0][key] = [
                np.round(a, decimals=3) for a in line
            ]
        for key in cls.calc["ALL"][0.0]:
            line = cls.calc["ALL"][0.0][key]
            cls.calc["ALL"][0.0][key] = [np.round(a, decimals=3) for a in line]
            cls.calc_dark["1 dark"][0.0][key] = [np.round(a, decimals=3) for a in line]

    def test_keys(self):
        self.assertEqual(sorted(self.calc.keys()), sorted(self.expected.keys()))
    ...
    def test_4_4(self):
        self.assertEqual(self.expected["ALL"][0.0]["4 4"], self.calc["ALL"][0.0]["4 4"])

    def test_4_5(self):
        self.assertEqual(self.expected["ALL"][0.0]["4 5"], self.calc["ALL"][0.0]["4 5"])

    def test_5_6(self):
        self.assertEqual(self.expected["ALL"][0.0]["5 6"], self.calc["ALL"][0.0]["5 6"])

    def test_6_6(self):
        self.assertEqual(self.expected["ALL"][0.0]["6 6"], self.calc["ALL"][0.0]["6 6"])

    def test_6_5(self):
        self.assertEqual(self.expected["ALL"][0.0]["6 5"], self.calc["ALL"][0.0]["6 5"])

    def test_5_5(self):
        self.assertEqual(self.expected["ALL"][0.0]["5 5"], self.calc["ALL"][0.0]["5 5"])

    def test_5_4(self):
        self.assertEqual(self.expected["ALL"][0.0]["5 4"], self.calc["ALL"][0.0]["5 4"])

    def test_6_3(self):
        self.assertEqual(self.expected["ALL"][0.0]["6 3"], self.calc["ALL"][0.0]["6 3"])

    def test_3_3(self):
        self.assertEqual(self.expected["ALL"][0.0]["3 3"], self.calc["ALL"][0.0]["3 3"])

    def test_6_4(self):
        self.assertEqual(self.expected["ALL"][0.0]["6 4"], self.calc["ALL"][0.0]["6 4"])

    def test_4_3(self):
        self.assertEqual(self.expected["ALL"][0.0]["4 3"], self.calc["ALL"][0.0]["4 3"])

    def test_3_2(self):
        self.assertEqual(self.expected["ALL"][0.0]["3 2"], self.calc["ALL"][0.0]["3 2"])

    def test_2_1(self):
        self.assertEqual(self.expected["ALL"][0.0]["2 1"], self.calc["ALL"][0.0]["2 1"])

    def test_1_1(self):
        self.assertEqual(self.expected["ALL"][0.0]["1 1"], self.calc["ALL"][0.0]["1 1"])

    def test_1_2(self):
        self.assertEqual(self.expected["ALL"][0.0]["1 2"], self.calc["ALL"][0.0]["1 2"])

    def test_4_4_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["4 4"],
            self.calc_dark["1 dark"][0.0]["4 4"],
        )

    def test_4_5_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["4 5"],
            self.calc_dark["1 dark"][0.0]["4 5"],
        )

    def test_5_6_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["5 6"],
            self.calc_dark["1 dark"][0.0]["5 6"],
        )

    def test_6_6_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["6 6"],
            self.calc_dark["1 dark"][0.0]["6 6"],
        )

    def test_6_5_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["6 5"],
            self.calc_dark["1 dark"][0.0]["6 5"],
        )

    def test_5_5_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["5 5"],
            self.calc_dark["1 dark"][0.0]["5 5"],
        )

    def test_5_4_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["5 4"],
            self.calc_dark["1 dark"][0.0]["5 4"],
        )

    def test_6_3_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["6 3"],
            self.calc_dark["1 dark"][0.0]["6 3"],
        )

    def test_3_3_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["3 3"],
            self.calc_dark["1 dark"][0.0]["3 3"],
        )

    def test_6_4_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["6 4"],
            self.calc_dark["1 dark"][0.0]["6 4"],
        )

    def test_4_3_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["4 3"],
            self.calc_dark["1 dark"][0.0]["4 3"],
        )

    def test_3_2_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["3 2"],
            self.calc_dark["1 dark"][0.0]["3 2"],
        )

    def test_2_1_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["2 1"],
            self.calc_dark["1 dark"][0.0]["2 1"],
        )

    def test_1_1_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["1 1"],
            self.calc_dark["1 dark"][0.0]["1 1"],
        )

    def test_1_2_dark(self):
        self.assertEqual(
            self.expected_dark["1 dark"][0.0]["1 2"],
            self.calc_dark["1 dark"][0.0]["1 2"],
        )

    def test_1_light(self):
        self.assertEqual(self.expected_dark["1 light"], self.calc_dark["1 light"])
...
class TestGetAntennaTransitions(unittest.TestCase):
    ...
    def test_all_phases(self):
        transitions = tr.get_antenna_transition_durations(
            self.data, self.timeline, binsize=30 * 60
        )

    def test_ALL(self):
        transitions = tr.get_antenna_transition_durations(
            self.data, self.timeline, binsize="ALL"
        )
