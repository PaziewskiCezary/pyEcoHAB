...
class TestGetStates(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.t_start = 600.0
        cls.t_end = 800.0
        cls.config1 = SetupConfig(path=data_path, fname="setup_short_2.txt")
        cls.config2 = SetupConfig(path=data_path, fname="setup_short_3.txt")
        cls.antennas = [
            "3",
            "4",
            "4",
        ...
        cls.out_1 = dom.get_states_mouse(
            cls.antennas, cls.times, cls.t_start, cls.t_end, cls.config1, cls.dt
        )
        cls.out_2 = dom.get_states_mouse(
            cls.antennas, cls.times, cls.t_start, cls.t_end, cls.config2, cls.dt
        )
...
class TestFindStimulusCageMice(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        out_1[45:100] = 3
        out_2[80:108] = 3
        cls.data = {"mouse 1": out_1, "mouse 2": out_2, "mouse 3": out_3}

    def test_1(self):
        t_start = 4.2
        t_stop = 6.05
        beginning = 2.1
        out = dom.find_stimulus_cage_mice(
            self.data, t_start, t_stop, beginning, self.dt
        )
...
class TestCheckMouse2NotValid(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.times = [11.0, 12.0, 13.0, 14.0, 21.0]

    def test_no_pre(self):
        self.assertTrue(
            dom.check_mouse2_not_valid(10, 20, ["3", "4"], [11.0, 12.0], "3")
        )

    def test_no_between(self):
        self.assertTrue(
            dom.check_mouse2_not_valid(15, 20, self.antennas, self.times, "3")
        )

    def test_not_home_antenna(self):
        self.assertTrue(
            dom.check_mouse2_not_valid(11.5, 20, self.antennas, self.times, "4")
        )

    def test_return_False(self):
        self.assertFalse(
            dom.check_mouse2_not_valid(11.5, 20, self.antennas, self.times, "3")
        )
...
class TestCountAttempts(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.config = SetupConfig(path=data_path, fname="setup_short_2.txt")

    def test_check_1_true(self):
        out = dom.count_attempts(
            12.5, 13.5, self.times, self.antennas, "4", self.config
        )
        self.assertEqual(out, 1)

    def test_check_1_false(self):
        out = dom.count_attempts(
            11.5, 12.5, self.times, self.antennas, "3", self.config
        )
        self.assertEqual(out, 0)

    def test_check_2_false(self):
        out = dom.count_attempts(
            11.5, 14.5, self.times, self.antennas, "3", self.config
        )
        self.assertEqual(out, 0)

    def test_check_2_true(self):
        out = dom.count_attempts(
            11.5, 13.5, self.times, self.antennas, "4", self.config
        )
        self.assertEqual(out, 1)

    def test_check_more(self):
        out = dom.count_attempts(
            11.5, 41.5, self.times, self.antennas, "4", self.config
        )
        self.assertEqual(out, 1)

    def test_check_more_opposite_antenna(self):
        out = dom.count_attempts(
            12.5, 41.5, self.times, self.antennas, "3", self.config
        )
...
class TestMouseDefending(unittest.TestCase):
    ...
    def test_mouse_1_in_sugar_cage(self):
        out = dom.check_mouse1_defending(
            self.antennas1, self.times1, self.antennas2, self.times2, "4", self.config
        )
        self.assertEqual(out, 1)
    ...
    def test_mouse_2_more_attempst(self):
        antennas1 = ["4", "4", "4", "4"]
        out = dom.check_mouse1_defending(
            antennas1, self.times1, self.antennas2, self.times2, "3", self.config
        )
        self.assertEqual(out, 3)
