...
def get_states_mouse(antennas, times, t_start, t_end, config, dt):
    ...
    if len(config.internal_antennas):
        provided = home_cage_internal_antennas + stimCage_internal_antennas
        unaccounted_for = list(set(config.internal_antennas) - set(provided))
    else:
    ...
    a_now = antennas[i]
    t_now = times[i]
    if a_now != home_antenna and antennas[0] not in home_cage_internal_antennas:
    ...
    while i + 1 < len(antennas):
        ...
        if next_a in stimCage_internal_antennas:
            ...
            if previous == 1:
                if a_now != home_antenna and a_now not in home_cage_internal_antennas:
                    states[timestamp:next_timestamp] = 3
            else:
                if next_t - t_now > 2:
                    if (
...
def get_states(ecohab_data, t_start, t_end, dt=0.05):
    """
    0 -- home cage, 1 -- pipe, 2 -- stimulus compartment
    """
    states = {}
    for mouse in ecohab_data.mice:
        times, antennas = utils.get_times_antennas(ecohab_data, mouse, t_start, t_end)
        states[mouse] = get_states_mouse(
            antennas, times, t_start, t_end, ecohab_data.stimulus_config, dt
        )
    return states
...
def get_dominating_mice(
    ecohab_data, timeline, phase, mouse, states, homecage_entrance, dt
):
    ...
    while True:
        if idx >= len(antennas):
            break
        if (
            antennas[idx] == homecage_entrance
            and antennas[idx - 1] == homecage_entrance
        ):
            mice_list = find_stimulus_cage_mice(
                states, time[idx - 1], time[idx], T_START, dt
            )
...
def dominating_mice(ecohab_data, timeline, phase, states, homecage_entrance, dt=0.05):
    ...
    for i, mouse in enumerate(ecohab_data.mice):
        results[:, i] = get_dominating_mice(
            ecohab_data, timeline, phase, mouse, states, homecage_entrance, dt=dt
        )
    return results


def tube_dominance_2_mice_single_phase(
    ecohab_data, mouse1, mouse2, t_start, t_end, homecage_entrance
):
    ...
    domination_counter = check_mouse1_defending(
        m1_antennas,
        m1_times,
        m2_antennas,
        m2_times,
...
def tube_dominance_2_cages(data, timeline, phase, hc_a):
    ...
    for i, mouse1 in enumerate(mice):
        for j, mouse2 in enumerate(mice):
            if i != j:
                dominance[i, j] = tube_dominance_2_mice_single_phase(
                    data, mouse1, mouse2, st, en, hc_a
                )
    return dominance


def count_attempts(tstamp1, tstamp2, times, antennas, homecage_entrance, config):
    ...
    while True:
        if (
            in_between_antennas[i] == homecage_entrance
...
def check_mouse1_not_valid(mouse_previous_antenna, mouse_antenna, homecage_entrance):
...
def check_mouse2_not_valid(
    mouse1_previous_timestamp, mouse1_timestamp, antennas2, times2, homecage_entrance
):
    ...
    if mouse2_pre is None:
        return True
    mouse2_between = utils.get_idx_between(
        mouse1_previous_timestamp, mouse1_timestamp, times2
    )
...
def check_mouse1_defending(
    antennas1, times1, antennas2, times2, homecage_entrance, config
):
    dominance_counter = 0
    for idx in range(1, len(antennas1)):
        ...
        if check_mouse1_not_valid(
            mouse1_previous_antenna, mouse1_antenna, homecage_entrance
        ):
            continue
        if check_mouse2_not_valid(
            mouse1_previous_timestamp,
            mouse1_timestamp,
            antennas2,
            times2,
            homecage_entrance,
            ...
            continue
        dominance_counter += count_attempts(
            mouse1_previous_timestamp,
            mouse1_timestamp,
            times2,
            antennas2,
...
def get_tube_dominance_2_cages(
    ecohab_data, timeline, res_dir=None, prefix=None, dt=0.05, delimiter=";"
):
    t_start, t_end = timeline.get_time_from_epoch("ALL")
    ...
    dispatch.evaluate_whole_experiment(
        ...
        "pushed out mouse",
        "# pushes",
        args=[ecohab_data.homecage_entrance],
        vmin=0,
        vmax=200,
        delimiter=delimiter,
    )
...
def get_subversion_evaluation(
    ...
    if prefix is None:
        prefix = ecohab_data.prefix
    states = get_states(ecohab_data, timeline, dt)
    dispatch.evaluate_whole_experiment(
        ecohab_data,
        timeline,
        res_dir,
        ...
        "subversive mouse",
        "# times in small cage",
        args=[states, ecohab_data.homecage_entrance, dt],
        vmin=0,
        vmax=200,
        delimiter=delimiter,
    )
...
def get_visits_to_stimulus_cage(
    ecohab_data, timeline, res_dir="", prefix="", dt=0.05, delimiter=";"
):
    if res_dir == "":
        res_dir = ecohab_data.res_dir
    if prefix == "":
    ...
    results = np.zeros((1, len(ecohab_data.mice), len(phases)))
    cumulative = np.zeros((1, len(ecohab_data.mice)))
    ...
    write_csv_alone(
        ...
        res_dir,
        prefix,
        labels=["conditioning compartment"],
