...
def generate_intervals(t_starts, t_stops, duration):
    intervals = sorted(
        utils.get_interval_durations_2_lists(t_starts, t_stops), reverse=True
    )
    ...
    while i < ints_len:
        ...
        out = insert_interval(
            can_t_start, interval, new_t_starts, new_t_stops, duration
        )
        iterations += 1
        i += out
...
def gen_directions_dict(directions_dict, duration, keys):
    ...
    for key in keys:
        ...
        new_dict[key] = generate_intervals(old_intervals[0], old_intervals[1], duration)
    return new_dict


def bootstrap_single_phase(directions_dict, mice_list, t_start, t_stop, keys, N=1000):
    followings = utils.make_results_dict(mice_list, tolist=True)
    times_together = utils.make_results_dict(mice_list, tolist=True)
    new_directions = {}
    for i in range(N):
        for mouse in mice_list:
            new_directions[mouse] = gen_directions_dict(
                directions_dict[mouse], t_stop - t_start, keys
            )
        out = following_matrices(new_directions, mice_list, t_start, t_stop, keys)
        for m1 in mice_list:
            for m2 in mice_list:
                if m1 != m2:
...
def resample_single_phase(
    ...
    followings, times_following = bootstrap_single_phase(
        directions_dict, mice, t_start, t_stop, keys, N=N
    )
    binsize = (t_stop - t_start) / 3600
    hist_dir = os.path.join(
        "other_variables", "dynamic_interactions_hists", "bin_%4.2f" % binsize
    )
    hist_time_dir = os.path.join(
        "other_variables", "durations_dynamic_interactions_hists", "bin_%4.2f" % binsize
    )
    ...
    if save_figures:
        for mouse1 in mice:
            for mouse2 in mice:
                ...
                fname1 = "%s%s_hist_%s_%s_N_%d_%4.2f" % (
                    prefix,
                    "DI",
                    phase.replace(" ", "_"),
                    key,
                ...
                fname2 = "%s%s_hist_%s_%s_N_%d_%4.2f" % (
                    prefix,
                    "durations_DI",
                    phase.replace(" ", "_"),
                    key,
    ...
    dist_dir_fol = os.path.join(
        "other_variables", "dynamic_interactions_hists", "bin_%4.2f" % binsize
    )
    dist_dir_time = os.path.join(
        "other_variables", "durations_dynamic_interactions_hists", "bin_%4.2f" % binsize
    )
...
def following_single_direction(intervals_m1, intervals_m2):
    ...
    for i in range(len(t_starts_m1)):
        indxs = utils.get_idx_between(t_starts_m1[i], t_ends_m1[i], t_starts_m2)
        for idx in indxs:
...
def get_dynamic_interactions(
    ...
    phases, times, data, data_keys = utils.get_registrations_bins(
    ...
    if isinstance(binsize, int) or isinstance(binsize, float):
        ...
        if int(binsize) == 24 * 3600:
            ...
            csv_results_following_exp = np.zeros((len(phases), len(mice), len(mice)))
            if save_times_following:
                csv_results_time = np.zeros((len(phases), len(mice), len(mice)))
                csv_results_time_exp = np.zeros((len(phases), len(mice), len(mice)))
    ...
    if full_dir_tree:
        fname_excess = "%s_%s_%s%s.csv" % (fname_beg, method, prefix, add_info_mice)
        fname_excess_rev = "%s_%s_%s%s.csv" % (fname_rev, method, prefix, add_info_mice)
        fname_rev_ = "leading_%s%s.csv" % (prefix, add_info_mice)
    else:
        fname_excess = "rasters_%s_%s_%s%s.csv" % (
            fname_beg,
            method,
            prefix,
            add_info_mice,
        )
        fname_excess_rev = "rasters_%s_%s_%s%s.csv" % (
            fname_rev,
            method,
            prefix,
            add_info_mice,
    ...
    if full_dir_tree:
        raster_dir = os.path.join(
            "dynamic_interactions", "raster_plots", "bins_%s" % binsize_name
        )
        raster_dir_add = os.path.join(
        ...
        hist_dir = os.path.join(
            "dynamic_interactions", "histograms", "bins_%s" % binsize_name
        )
        hist_dir_add = os.path.join(
            "dynamic_interactions",
            "additionals",
            "histograms",
            "bins_%s" % binsize_name,
        ...
        other_hist = os.path.join(
            "other_variables",
            "histograms_of_dynamic_interactions_intervals",
            "bins_%s" % binsize_name,
        )
        other_excess_hist = os.path.join(
            "other_variables",
            "dynamic_interactions_excess_histograms",
            "bins_%s" % binsize_name,
        )
        other_raster_dir = os.path.join(
            "other_variables",
            "durations_dynamic_interactions",
            "rasters",
            "bin_%s" % binsize,
        ...
        other_dir = os.path.join("other_variables", "durations_DI")
        other_hist = os.path.join("other_variables", "histograms_of_DI_intervals")
        other_excess_hist = os.path.join("other_variables", "DI_excess_histograms")
        other_raster_dir = os.path.join("other_variables", "durations_DI")
    meas_prefix = "measured_dynamic_interactions_%s_%s" % (prefix, add_info_mice)
    exp_prefix = "expected_dynamic_interactions_%s_%s" % (prefix, add_info_mice)
    excess_prefix = "excess_dynamic_interactions_%s_%s" % (prefix, add_info_mice)
    ...
    for idx_phase, ph in enumerate(all_phases):
        ...
        mouse_leading_sum[ph] = utils.sum_per_mouse(
            following[ph], mice, bin_labels[ph], "leader"
        )
        mouse_following_sum[ph] = utils.sum_per_mouse(
            following[ph], mice, bin_labels[ph], "follower"
        )
        mouse_leading_sum_div_activ[ph] = utils.divide_sum_activity(
            mouse_leading_sum[ph], mouse_activity[ph]
        )
        mouse_following_sum_div_activ[ph] = utils.divide_sum_activity(
            mouse_following_sum[ph], mouse_activity[ph]
        )
        if full_dir_tree:
        ...
        excess_following = utils.calc_excess(following[ph], following_exp[ph])

        mouse_leading_sum_excess[ph] = utils.sum_per_mouse(
            following_exp[ph], mice, bin_labels[ph], "leader"
        )
        mouse_following_sum_excess[ph] = utils.sum_per_mouse(
            following_exp[ph], mice, bin_labels[ph], "follower"
        )
        mouse_leading_sum_div_activ_excess[ph] = utils.divide_sum_activity(
            mouse_leading_sum_excess[ph], mouse_activity[ph]
        )
        mouse_following_sum_div_activ_excess[ph] = utils.divide_sum_activity(
            mouse_following_sum_excess[ph], mouse_activity[ph]
        )
        ...
        if isinstance(binsize, int) or isinstance(binsize, float):
            if int(binsize) == 24 * 3600:
                ...
                res = utils.dict_to_array_2D(following[ph][0], mice, mice)
                exp_res = utils.dict_to_array_2D(following_exp[ph][0], mice, mice)
                single_in_cohort_soc_plot(
                    res,
                    exp_res,
                    mice,
        ...
        phase_full_results = utils.dict_to_array_3D(
            following[ph], bin_labels[ph], mice, mice
        )
        phase_exp_full_results = utils.dict_to_array_3D(
            following_exp[ph], bin_labels[ph], mice, mice
        )
        write_csv_rasters(
            mice,
            raster_labels,
        ...
        if save_times_following:
            ...
            if isinstance(binsize, int) or isinstance(binsize, float):
                if int(binsize) == 24 * 3600:
                    ...
                    exp_res = utils.dict_to_array_2D(
                        time_together_exp[ph][0], mice, mice
                    )
                    single_in_cohort_soc_plot(
                        res,
                        exp_res,
                        ...
                        vmax1=vmax1t,
                        titles=[
                            "Fraction of duration dynamics interation",
                            "# expected duration",
                            "# excess duration",
    ...
    write_sum_data(
        mouse_leading_sum_div_activ_excess,
        "mouse_leading_activity_excess_%s" % binsize_name,
        mice,
        bin_labels,
        all_phases,
        res_dir,
    ...
    write_sum_data(
        mouse_following_sum_div_activ_excess,
        "mouse_following_activity_excess_%s" % binsize_name,
        mice,
        bin_labels,
        all_phases,
        res_dir,
    ...
    if save_times_following:
        ...
        write_interpair_intervals(
            ...
            other_hist,
            res_dir,
            "dynamic_interactions_intervals",
            prefix,
            additional_info=add_info_mice,
            delimiter=delimiter,
            full_dir_tree=full_dir_tree,
