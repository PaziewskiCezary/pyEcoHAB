...
def find_light_beginning(dark_beg, dark_len):
    ...
    return "%s:%s" % (convert_int_to_time(new_hours), convert_int_to_time(new_mins))
...
def gen_timeline(
    ...
    light_beginning = find_light_beginning(dark_beginning, dark_length)
    ...
    while True:
        ...
        config[full_phase_name] = make_config_entry(start_date, end_date)
        ...
        if start_date > total_end:
            config["ALL"] = make_config_entry(total_beg, end_date)
            break
    return config
