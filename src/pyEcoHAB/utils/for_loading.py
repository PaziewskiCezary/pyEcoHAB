...
def parse_fname(fname):
    ...
    datenext = time.strftime("%Y%m%d", time.localtime(date_in_sec + 24 * 3600.0))
...
def time_to_sec(tt):
    ...
    try:
        ...
        return calendar.timegm(time.strptime(tt + " UTC", "%Y%m%d %H:%M:%S %Z"))

    seconds = calendar.timegm(
        time.strptime(more_than_sec + " UTC", "%Y%m%d %H:%M:%S %Z")
    )
...
def check_antenna_presence(raw_data, setup_config, max_break):
    ...
    for antenna in setup_config.all_antennas:
        ...
        if len(times):
            ...
            breaks[antenna].append([np.round(t_start), raw_data["Time"][-1]])
    return breaks
...
def skipped_registrations(raw_data, setup_config):
    ...
    mice = set(raw_data["Tag"])
    mismatches = OrderedDict(
        [("skipped one", 0), ("skipped two", 0), ("skipped more", 0)]
    )
    for mouse in mice:
        mouse_idx = np.where(np.array(raw_data["Tag"]) == mouse)[0]
        ant = raw_data["Antenna"][mouse_idx]
...
def save_skipped_registrations(
    skipped,
    tot_registrations,
    res_dir,
...
def save_mismatches(
    ...
    header="antenna pair,  count, percentage\n",
):
    out_f1 = header
    for pair in mismatches.keys():
        a1, a2 = pair.split(" ")
        ...
        out_f1 += "%s, %d, %3.2f per 100\n" % (pair, mismatches[pair], exact_mis)
    new_path = check_directory(res_dir, "diagnostics")
    fpath1 = os.path.join(new_path, fname)
    f1 = open(fpath1, "w")
    f1.write(out_f1)
...
def save_total_mismatches(tot_mismatches, counters, res_dir):
    ...
    for a1 in tot_mismatches.keys():
        if counters[a1]:
            ...
            exact_mis = 0
        out_f1 += "%s, %d, %3.2f per 100\n" % (a1, tot_mismatches[a1], exact_mis)
    new_path = check_directory(res_dir, "diagnostics")
...
def run_diagnostics(raw_data, max_break, res_dir, setup_config):
    ...
    string_1 = save_mismatches(mismatches, len(raw_data["Antenna"]), res_dir)
...
def incorrect_tunnel_registrations(raw_data, setup_config):
    ...
    for mouse in mice:
        ...
        out_count, out_tot_count = incorrect_tunnel_single_mouse(
            count.keys(), antennas1, times1, durations1
        )
        for key in count:
...
def transform_raw(row):
    return (int(row[0]), time_to_sec(row[1]), row[2], int(row[3]), row[4])
