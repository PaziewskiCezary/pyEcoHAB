...
def write_single_chamber(
    f, header, phases, mice, time, data_stim, delimiter, floats=False
):
    ...
    for i, mouse in enumerate(mice):
        ...
        for phase in phases:
            for k, t in enumerate(time[phase]):
                ...
                try:
                    if floats:
                        lines[k] += "%s%7.3f" % (delimiter, data_stim[phase][mouse][k])
                    else:
...
def save_data_cvs(
    ...
    for stim in which:
        for j, h in enumerate(headers):
            f.write("%s %s\n" % (h, stim))
            write_single_chamber(
                f, head, phases, mice, bin_labels, data[stim][j], delimiter, floats=j
            )


def write_binned_data(
    data_stim,
    fname,
    mice,
    bin_labels,
    ...
    if full_dir_tree:
        ...
        new_path = os.path.join(path, target_dir)
    fname = os.path.join(
        new_path, "%s_%s_%s_%s.csv" % (fname, prefix, phase, additional_info)
    )
    if not os.path.exists(new_path):
        os.makedirs(new_path)
    logging.info('Creating %s', fname)
...
def write_csv_rasters(
    ...
    if symmetrical:
        ...
        new_output, pairs = utils.make_table_of_all_mouse_pairs(
            output, phases, mice, reverse
        )
...
def write_csv_tables(
    results, phases, mice, main_directory, dirname, fname, prefix, delimiter=";"
):
    new_name = os.path.join(dirname, "data")
    directory = utils.check_directory(main_directory, new_name)
    ...
    phase_pairs = [
        phases[2 * i] + (len(mice) + 2) * delimiter + phases[2 * i + 1] + "\n"
        for i in range(len(phases) // 2)
    ]
    new_results = [
        (results[2 * i], results[2 * i + 1]) for i in range(len(phases) // 2)
    ]
    mice_header = ""
    mice_len = 2
    if len(phases) == 1:
...
def write_interpair_intervals(
    ...
    directory = utils.check_directory(directory, new_name)
    fname = os.path.join(directory, "%s_%s_%s.csv" % (fname, prefix, additional_info))
    try:
        f = open(fname, "w")
    except IOError:
        print("Could not write to file", fname)
...
def write_bootstrap_results(
    ...
    new_name = os.path.join(
        new_dir, "%s_%s_%s_%s.csv" % (fname, phase.replace(" ", "_"), prefix, add_info)
    )
...
def save_antenna_transitions(
    transition_times, fname, res_dir, prefix, directory, delimiter=";"
):
